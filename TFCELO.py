...
async def startserver(ctx, region: str):
    async with GLOBAL_LOCK:
        ...
        instance_state = client.describe_instance_status(InstanceIds=[logins[region]["instance_id"]])['InstanceStatuses'][0]['InstanceState']['Name']
        ...
        if channel.name == v["pc"]:
            ...
            if server_vote == 1:
                ...
                if winningIP.find('aws') != '-1':
                    ...
                    instance_state = client.describe_instance_status(InstanceIds=[logins[server]["instance_id"]])['InstanceStatuses'][0]['InstanceState']['Name']
                    if instance_state != 'running':
                        await ctx.send(f"{server} server is not running! Starting it up now")
                        client.start_instances(InstanceIds=[logins[server]["instance_id"]])
                await voteSetup(ctx)
            elif server_vote == 0:
                map_vote_message_view.stop()
