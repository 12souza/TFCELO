    ...
    for line in Lines:
        try:
            if (matchbegin == 0):
                if ("joined" in line):
                    ...
                    if (team == "Blue" or team == "Red"):
                        if (team == "Blue"):
                            blueTeam.append(steamid)
                        if (team == "Red"):
                            redTeam.append(steamid)
            ...
            if ("killed" in line and matchbegin == 1):
                ...
                if (victeam != team):
                    ...
                    if (team == 'Blue'):
                        ...
                        kills[steamid][2] += 1
                    if (currentClass[steamid] == "Soldier"):
                        kills[steamid][4] += 1
                    elif (currentClass[steamid] == "Demoman"):
                        kills[steamid][5] += 1
                    elif (currentClass[steamid] == "Medic"):
                        kills[steamid][6] += 1
                    elif (currentClass[steamid] == "HWGuy"):
                        kills[steamid][7] += 1
                    elif (currentClass[steamid] == "Spy"):
                        kills[steamid][8] += 1
                    elif (currentClass[steamid] == "Engineer"):
                        kills[steamid][9] += 1
                    if (weaponname == 'sentrygun'):
                        kills[steamid][11] += 1
                    if (weaponname == 'knife'):
                        kills[steamid][12] += 1
            ...
            if ("[SUMMARY]" in line):
                ...
                dmg = int(line[b:e])
                if (team == "Blue"):
                    damage[steamid][2] += dmg
                if (team == "Red"):
                    damage[steamid][1] += dmg
...
async def refreshlb(ctx):
    ...
    for i in list(totalKills):
        ...
        if (x < 4):
            msgList.append(f'{kills[i][13]}{space * (30 - len(kills[i][13]))}{totalKills[i]}\n')
        elif (x == 4):
            msgList.append(f'{kills[i][13]}{space * (30 - len(kills[i][13]))}{totalKills[i]}\n')
        x += 1
        if (x == 5):
...
async def hltv_ftp(region: str, map_name: str) -> tuple[ZipFile, list]:
    ...
    try:
        ...
        with ZipFile(file=demo_zip, compression=zipfile.ZIP_DEFLATED, mode='w') as zip_object:
            zip_object.write(demo_to_upload_1)
            zip_object.write(demo_to_upload_2)

        local_file_list.append(demo_zip)
        return demo_zip, local_file_list
    ...
    if (region.lower() == "none"):
        ...
        try:
            ...
            for logFile in logListNameDesc:
                ...
                if ((size > MIN_LOG_FILE_SIZE) and (".log" in logFile)): # Rounds with logs of players and time will be big
            ...
            try:
                ...
                print("Error retrieving log files to save locally", e)
            ...
            demo_zip, hltv_local_file_list = await hltv_ftp(region, pMap)
            if demo_zip is not None:
                local_file_list.extend(hltv_local_file_list)
                await schannel.send(file=discord.File(demo_zip), content=f"**Hampalyzer:** {hampa} {pMap} {pDate} {region}")
            else:
                await schannel.send(content=f"**Hampalyzer:** {hampa} {pMap} {pDate} {region} (HLTV DOWNLOAD FAILED)")
            ...
            try:
                ...
                print("refreshing leaderboard failed", e)
            local_file_list.append(logToParse1)
            local_file_list.append(logToParse2)
            for input_file in local_file_list:
                os.remove(input_file)
        except Exception as e:
            await ctx.send(f"{e}")
